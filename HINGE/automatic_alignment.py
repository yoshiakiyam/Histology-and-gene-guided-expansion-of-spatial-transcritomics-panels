...
def icp_affine_robust(source, target, n_iter=100, tol=1e-7, trim_frac=0.85):
    ...
    # Track cumulative transform as 3x3 homogeneous matrix
    # Represents the full transform FROM source (coarse-aligned input) TO registered
    A_cumulative_3x3 = np.eye(3)
    ...
    for iteration in range(n_iter):
        ...
        # A is (3,2): [x, y, 1] @ A = [x', y']

        # Accumulate into 3x3 homogeneous matrix
        # A_3x3: last column is [0, 0, 1] to preserve homogeneous coord
        A_3x3 = np.eye(3)
        A_3x3[:, :2] = A           # fill first two columns
        A_cumulative_3x3 = A_cumulative_3x3 @ A_3x3

        src = src_h @ A

        err = np.median(dist[keep_idx])
        if abs(prev_err - err) < tol:
            print(f"    [icp] converged at iteration {iteration+1}, "
                  f"median dist (inliers): {err:.2f}")
            break
    ...
    # Extract (3,2) cumulative matrix for use with apply_affine()
    A_cumulative_3x2 = A_cumulative_3x3[:, :2]

    return src, A_cumulative_3x2
...
def update_coordinates(registration_results, pairs,
    ...
    for pair_idx, (name_panel1, name_panel2) in enumerate(pairs):
        ...
        if pre_aligned:
            ...
            uni_panel2_list[pair_idx].obsm['spatial_registered'] =  uni_panel2_list[pair_idx].obsm['spatial'].copy()
            ...
            uni_panel2_list[pair_idx].obsm['spatial_registered'] = xy_reg.copy()
            
            rna_data[name_panel2].obsm['spatial_registered'] = xy_reg.copy()
...
def plot_alignment_results(pairs, rna_data, registration_results,
    ...
    for name_panel1, name_panel2 in pairs:
        ...
        # ── Panel 1: Before ───────────────────────────────────────
        ax = axes[0]
        ax.scatter(*xy_panel1[i_panel1].T, s=1, alpha=0.5, c='royalblue',
                   label=f'{name_panel1} ({panel1_name} panel)')
        ax.scatter(*xy_panel2[i_panel2].T,   s=1, alpha=0.5, c='tomato',
        ...
        ax.scatter(*xy_panel1[i_panel1].T, s=1, alpha=0.5, c='royalblue',
                   label=f'{name_panel1} ({panel1_name} panel)')
        ax.scatter(*xy_reg[i_panel2].T,  s=1, alpha=0.5, c='tomato',
